...
exports.calculate_troops = calculate_troops;
exports.attack = attack;
exports.calculate_damage = calculate_damage;
...
function calculate_troops(player, energy = FULL_ENERGY, war, drones = false) {
...
async function attack(user, war, agressor = false, max = false, drones = false) {
    try {
        const free_ene = max ? 0 : 1;
        const calculatedTroops = calculate_troops(user.player, FULL_ENERGY, war, drones);
...
function calculate_damage(player, war, agressor = false) {
    ...
    const alpha = Object.keys(troops).reduce((acc, troop) => acc + troops[troop] * TROOP_ADMG[troop], 0);
    const tanks_ratio = (troops['tanks'] * TROOP_ADMG['tanks']) / alpha;
    const ships_ratio = (troops['battleships'] * TROOP_ADMG['battleships']) / alpha;
    const space_ratio = (troops['spaceStations'] * TROOP_ADMG['spaceStations']) / alpha;
    const drone_ratio = (troops['laserDrones'] * TROOP_ADMG['laserDrones']) / alpha;
    const troop_bonus = 1 +
