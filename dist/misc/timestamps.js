export function getTimestamp(text) {
    ...
    if (match) {
        ...
        if (dateTimeStr.includes('today')) {
            ...
            dateTimeStr = dateTimeStr.replace('tomorrow', `${tomorrow.getUTCDate()} ${tomorrow.toLocaleString('default', {
                month: 'long',
    ...
    for (let i = 0; i < formatParts.length; i++) {
        ...
        switch (part) {
            case 'dd':
                ...
                break;
            case 'MMMM':
                date.setUTCMonth(new Date(Date.parse(parts[i] + ' 1, 2000')).getMonth());
                break;
            case 'MM':
